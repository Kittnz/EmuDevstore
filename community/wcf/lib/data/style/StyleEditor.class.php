 ...
 */
class StyleEditor extends Style {
	const INFO_FILE = 'style.xml';
	const STYLE_PREVIEW_IMAGE_MAX_WIDTH = 185;
	const STYLE_PREVIEW_IMAGE_MAX_HEIGHT = 140;
	
	/**
	 * Updates the data of this style.
	 */
	public function update($styleName, $variables = null, $templatePackID = null, $description = null, $version = null, $date = null, $image = null, $copyright = null, $license = null, $authorName = null, $authorURL = null, $disabled = null) {
		$imageChanged = ($image != $this->image);
		
		// update values
		$this->data['styleName'] = $styleName;
		if ($templatePackID !== null) $this->data['templatePackID'] = $templatePackID;
		if ($description !== null) $this->data['styleDescription'] = $description;
		if ($version !== null) $this->data['styleVersion'] = $version;
		if ($date !== null) $this->data['styleDate'] = $date;
		if ($image !== null) $this->data['image'] = $image;
		if ($copyright !== null) $this->data['copyright'] = $copyright;
		if ($license !== null) $this->data['license'] = $license;
		if ($authorName !== null) $this->data['authorName'] = $authorName;
		if ($authorURL !== null) $this->data['authorURL'] = $authorURL;
		if ($disabled !== null) $this->data['disabled'] = $disabled;
		
		// update database entry
		$sql = "UPDATE	wcf".WCF_N."_style
			SET	styleName = '".escapeString($styleName)."'
				".($templatePackID !== null ? ",templatePackID = ".$templatePackID : "")."
				".($description !== null ? ",styleDescription = '".escapeString($description)."'" : "")."
				".($version !== null ? ",styleVersion = '".escapeString($version)."'" : "")."
				".($date !== null ? ",styleDate = '".escapeString($date)."'" : "")."
				".($image !== null ? ",image = '".escapeString($image)."'" : "")."
				".($copyright !== null ? ",copyright = '".escapeString($copyright)."'" : "")."
				".($license !== null ? ",license = '".escapeString($license)."'" : "")."
				".($authorName !== null ? ",authorName = '".escapeString($authorName)."'" : "")."
				".($authorURL !== null ? ",authorURL = '".escapeString($authorURL)."'" : "")."
				".($disabled !== null ? ",disabled = ".$disabled : "")."
			WHERE	styleID = ".$this->styleID;
		WCF::getDB()->sendQuery($sql);
		
		// save variables
		if ($variables !== null) $this->setVariables($variables);
		
		// scale preview image
		if (!empty($image) && $imageChanged) {
			self::scalePreviewImage(WCF_DIR.$image);
		}
	...
	public static function import($filename, $packageID = PACKAGE_ID, $style = null) {
		...
		if (FileUtil::getSafeMode() && !empty($data['images']) && !file_exists(WCF_DIR.$imagesLocation)) {
			...
			if (strpos($data['variables']['page.logo.image'], '../') !== false) $data['variables']['page.logo.image'] = 'images/'.basename($data['variables']['page.logo.image']);
		}
		...
		if (!empty($data['templates'])) {
			...
			if (empty($templatePackFolderName)) $templatePackFolderName = 'generic'.StringUtil::substring(StringUtil::getRandomID(), 0, 8);
	...
	public function export($templates = false, $images = false, $icons = false) {
		...
		if ($templates && $this->templatePackID) $string .= "\t\t<templates>templates.tar</templates>\n"; // templates
	...
	public function setVariables($variables) {
		...
		foreach ($variables as $name => $value) {
			...
			$inserts .= "(".$this->styleID.", '".escapeString($name)."', '".escapeString($value)."')";
		}
	...
	public function delete() {
		// delete style
		$sql = "DELETE FROM	wcf".WCF_N."_style
			WHERE		styleID = ".$this->styleID;
		WCF::getDB()->sendQuery($sql);
	 ...
	 * Creates a new style.
	 * 
	 * @return	StyleEditor	new style
	 */
	public static function create($styleName, $variables, $templatePackID = 0, $description = null, $version = null, $date = null, $image = null, $copyright = null, $license = null, $authorName = null, $authorURL = null, $disabled = null, $packageID = PACKAGE_ID) {
		// default date
		if ($date === null) $date = gmdate('Y-m-d', TIME_NOW);
		if ($disabled === null) $disabled = 0;
		
		// save style
		$sql = "INSERT INTO	wcf".WCF_N."_style
					(styleName, templatePackID, packageID
					".($description != null ? ",styleDescription" : "")."
					".($version != null ? ",styleVersion" : "")."
					".($date != null ? ",styleDate" : "")."
					".($image != null ? ",image" : "")."
			...
			VALUES		('".escapeString($styleName)."', ".$templatePackID.", ".$packageID."
					".($description != null ? ",'".escapeString($description)."'" : "")."
					".($version != null ? ",'".escapeString($version)."'" : "")."
					".($date != null ? ",'".escapeString($date)."'" : "")."
					".($image != null ? ",'".escapeString($image)."'" : "")."
					".($copyright != null ? ",'".escapeString($copyright)."'" : "")."
					".($license != null ? ",'".escapeString($license)."'" : "")."
					".($authorName != null ? ",'".escapeString($authorName)."'" : "")."
					".($authorURL != null ? ",'".escapeString($authorURL)."'" : "")."
					".($disabled != null ? ",".$disabled : "").")";
		WCF::getDB()->sendQuery($sql);
		
		// get new style
		$styleID = WCF::getDB()->getInsertID();
		$style = new StyleEditor($styleID);
		
		// save variables
		$style->setVariables($variables);
