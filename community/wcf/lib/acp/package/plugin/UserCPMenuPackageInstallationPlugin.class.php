...
/**
 * This PIP installs, updates or deletes user cp menu items.
 * 
 ...
 */
class UserCPMenuPackageInstallationPlugin extends AbstractXMLPackageInstallationPlugin {
	public $tagName = 'usercpmenu';
	public $tableName = 'usercp_menu_item';
				...
				if ($block['name'] == 'import') {
					...
					foreach ($block['children'] as $acpMenuItem) {
						...
						if (!isset($acpMenuItem['attrs']['name'])) {
							throw new SystemException("Required 'name' attribute for usercp menu item is missing", 13023);
						}
						...
						if (!empty($parentMenuItem)) {
							$sql = "SELECT	COUNT(*) AS count
								FROM 	wcf".WCF_N."_usercp_menu_item
						...
						$sql = "INSERT INTO			wcf".WCF_N."_usercp_menu_item
				...
				// Handle the delete instructions.
					...
					if ($this->installation->getAction() == 'update') {
						...
						foreach ($block['children'] as $acpMenuItem) {
							// check required attributes
							if (!isset($acpMenuItem['attrs']['name'])) {
								throw new SystemException("Required 'name' attribute for 'usercpmenuitem'-tag is missing.", 13023);
							}
						...
						// Delete items.
						if (!empty($itemNames)) {
							$sql = "DELETE FROM	wcf".WCF_N."_usercp_menu_item
