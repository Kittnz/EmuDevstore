 ...
 */
class UserSendNewPasswordAction extends AbstractAction {
	/**
	 * @see Action::execute()
	 */
	public function execute() {
		...
		// check permission
		WCF::getUser()->checkPermission('admin.user.canEditPassword');
		...
		if (count($userIDs) > 0) {
			...
			WCF::getTPL()->assign(array(
				'pageTitle' => WCF::getLanguage()->get('wcf.acp.user.sendNewPassword'),
				'url' => 'index.php?action=UserNewPasswordMail&mailID='.$mailID.'&packageID='.PACKAGE_ID.SID_ARG_2ND_NOT_ENCODED
			));
