...
{if $userID|isset && $this->user->userID == $userID}
...
<div class="contentHeader">
	<div class="largeButtons">
		<ul>
			<li><a href="index.php?page=UserList&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}" title="{lang}wcf.acp.menu.link.user.list{/lang}"><img src="{@RELATIVE_WCF_DIR}icon/usersM.png" alt="" /> <span>{lang}wcf.acp.menu.link.user.list{/lang}</span></a></li>
...
<form method="post" action="index.php?form=User{@$action|ucfirst}">
	<div class="border content">
		<div class="container-1">
			...
			{if $action == 'add' || $this->user->getPermission('admin.user.canEditMailAddress')}
				<fieldset>
			...
			{if $action == 'add' || $this->user->getPermission('admin.user.canEditPassword')}
				<fieldset>
			...
			{if $options|count || $additionalTabs|isset}
				<div class="tabMenu">
					<ul>
						{foreach from=$options item=categoryLevel1}
							<li id="{@$categoryLevel1.categoryName}"><a onclick="tabMenu.showSubTabMenu('{@$categoryLevel1.categoryName}');"><span>{lang}wcf.user.option.category.{@$categoryLevel1.categoryName}{/lang}</span></a></li>
						{/foreach}
				...
				{foreach from=$options item=categoryLevel1}
					<div class="border tabMenuContent hidden" id="{@$categoryLevel1.categoryName}-content">
						<div class="container-1">
							<h3 class="subHeadline">{lang}wcf.user.option.category.{@$categoryLevel1.categoryName}{/lang}</h3>
							
							{foreach from=$categoryLevel1.categories item=categoryLevel2}
								<fieldset>
									<legend>{lang}wcf.user.option.category.{@$categoryLevel2.categoryName}{/lang}</legend>
									
									{if $categoryLevel2.categoryName == 'settings.general' && $availableLanguages|count > 1}
