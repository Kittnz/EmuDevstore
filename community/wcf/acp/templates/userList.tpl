...
<div class="contentHeader">
	{pages print=true assign=pagesLinks link="index.php?page=UserList&pageNo=%d&searchID=$searchID&action=$encodedAction&sortField=$sortField&sortOrder=$sortOrder&packageID="|concat:PACKAGE_ID:SID_ARG_2ND_NOT_ENCODED}
	...
	<div class="largeButtons">
		<ul>
			...
			{/if}
			<li><a href="index.php?form=UserSearch&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}" title="{lang}wcf.acp.user.search{/lang}"><img src="{@RELATIVE_WCF_DIR}icon/searchM.png" alt="" /> <span>{lang}wcf.acp.user.search{/lang}</span></a></li>
...
<div class="subTabMenu">
	<div class="containerHead">
		<ul>
			<li{if $action == ''} class="activeSubTabMenu"{/if}><a href="index.php?page=UserList&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}"><span>{lang}wcf.acp.user.list.all{/lang}</span></a></li>
			...
			{foreach from=$users item=user}
				<tr class="{cycle values="container-1,container-2" advance=false}" id="userRow{@$user->userID}">
					...
					<td class="columnIcon">
						...
						{if $user->editable}
							<a href="index.php?form=UserEdit&amp;userID={@$user->userID}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}"><img src="{@RELATIVE_WCF_DIR}icon/editS.png" alt="" title="{lang}wcf.acp.user.edit{/lang}" /></a>
						{else}
							<img src="{@RELATIVE_WCF_DIR}icon/editDisabledS.png" alt="" title="{lang}wcf.acp.user.edit{/lang}" />
						{/if}
						{if $user->deletable}
							<a onclick="return confirm('{lang}wcf.acp.user.delete.sure{/lang}')" href="index.php?action=UserDelete&amp;userID={@$user->userID}&amp;url={@$encodedURL}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}"><img src="{@RELATIVE_WCF_DIR}icon/deleteS.png" alt="" title="{lang}wcf.acp.user.delete{/lang}" /></a>
						{else}
					...
					<td class="columnUsername columnText">{if $user->editable}<a title="{lang}wcf.acp.user.edit{/lang}" href="index.php?form=UserEdit&amp;userID={@$user->userID}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}">{$user->username}</a>{else}{$user->username}{/if}</td>
