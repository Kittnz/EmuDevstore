...
<script type="text/javascript" src="{@RELATIVE_WCF_DIR}acp/js/PackageNameEditor.class.js"></script>
<script type="text/javascript">
	//<![CDATA[
	var packageNameEditor = new PackageNameEditor();
	//]]>
</script>
...
<div class="contentHeader">
	...
	{if $this->user->getPermission('admin.system.package.canInstallPackage') || $additionalLargeButtons|isset}
		<div class="largeButtons">
			<ul>
				{if $this->user->getPermission('admin.system.package.canInstallPackage')}<li><a href="index.php?form=PackageStartInstall&amp;action=install&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}" title="{lang}wcf.acp.package.startInstall{/lang}"><img src="{@RELATIVE_WCF_DIR}icon/packageInstallM.png" alt="" /> <span>{lang}wcf.acp.package.startInstall{/lang}</span></a></li>{/if}
				...
				{foreach from=$packages item=$package}
					<tr class="{cycle values="container-1,container-2"}">
						...
						<td class="columnText" title="{$package.packageDescription}" id="packageName{@$package.packageID}">
							<a href="index.php?page=PackageView&amp;activePackageID={@$package.packageID}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}"><span>{$package.packageName}{if $package.instanceNo > 1 && $package.instanceName == ''} (#{#$package.instanceNo}){/if}</span></a>
							
							<script type="text/javascript">
								//<![CDATA[
								packageNameEditor.init({@$package.packageID});
								//]]>
							</script>
						</td>
						...
						<td class="columnDate">{@$package.updateDate|shorttime}</td>
