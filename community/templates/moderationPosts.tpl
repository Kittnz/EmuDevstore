		...
		<div class="container-1">
			...
			{else}
				...
				<div class="contentHeader">
					{pages print=true assign=pagesOutput link="index.php?page=$page&pageNo=%d"|concat:SID_ARG_2ND_NOT_ENCODED}
				...
				<script type="text/javascript">
					...
					var url = 'index.php?page={@$page}&pageNo={@$pageNo}{@SID_ARG_2ND_NOT_ENCODED}';
					//]]>
				</script>
						...
						<div class="messageInner container-{cycle name='container'}">
							<div class="messageHeader">
								...
								<div class="containerContent">
									<p class="light smallFont">{@$post->time|time} - {lang}wbb.board.threads.postBy{/lang} {if $post->userID}<a href="index.php?page=User&amp;userID={@$post->userID}{@SID_ARG_2ND}">{$post->username}</a>{else}{$post->username}{/if}</p>
								</div>
							</div>
				...
				<div class="contentFooter">
					...
					{if $action == 'deletedPosts'}
						<div class="largeButtons">
							<ul>
								<li><a href="index.php?action=EmptyPostRecycleBin&amp;t={@SECURITY_TOKEN}{@SID_ARG_2ND}" onclick="return confirm('{lang}wbb.moderation.emptyRecycleBin.sure{/lang}')" title="{lang}wbb.moderation.emptyRecycleBin{/lang}"><img src="{icon}emptyRecycleBinM.png{/icon}" alt="" /> <span>{lang}wbb.moderation.emptyRecycleBin{/lang}</span></a></li>
							</ul>
						</div>
					{/if}
				</div>
